<div>
    ...
    @foreach ($current_component->items as $key => $value)
        <div class="mt-5" class="flex">
            <x-input-label for="{{ $parent_path }}.{{ $current_component->name }}.{{ $key }}" value="{{ ucfirst($current_component->name) }} {{ $key }}" />
            <textarea id="{{ $parent_path }}.{{ $current_component->name }}.{{ $key }}" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" wire:model.live="current_component.items.{{ $key }}"></textarea>
